...
install-all:
ifdef PROJECT
	cd scripts/install && uv run python -m cq_install install --target codex --target opencode --target cursor --target claude --target pi --project "$(PROJECT)"
	@echo "Note: Windsurf has no per-project MCP config; installing it globally."
	cd scripts/install && uv run python -m cq_install install --target windsurf --global
else
	cd scripts/install && uv run python -m cq_install install --target codex --target opencode --target cursor --target windsurf --target claude --target pi --global
endif
